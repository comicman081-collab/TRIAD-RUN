(function(global){
  ...
  const CARD_PROJECTILE_PROFILES=Object.freeze({
    quick:{duration:680,contactMs:490,particleProfile:'needle',impactAssetKey:'SHOCK',impactProfile:'ion-burst',emphasis:.86},
    dot:{duration:840,contactMs:605,particleProfile:'ember-orb',impactAssetKey:'BURN',impactProfile:'ember-plume',emphasis:1.04},
    volley:{duration:710,contactMs:512,particleProfile:'scatter',impactAssetKey:'SHOCK',impactProfile:'scatter-volley',emphasis:.78},
    ambush:{duration:735,contactMs:529,particleProfile:'shade-needle',impactAssetKey:'MARK',impactProfile:'void-seal',emphasis:.92},
    mark:{duration:790,contactMs:569,particleProfile:'lockshot',impactAssetKey:'MARK',impactProfile:'lock-on-seal',emphasis:.94}
  });
  const ENEMY_PROJECTILE_PROFILES=Object.freeze({
    SCOUT:{duration:700,contactMs:504,particleProfile:'enemy-needle',impactAssetKey:'SHOCK',impactProfile:'ion-burst',emphasis:.86},
    CASTER:{duration:880,contactMs:634,particleProfile:'enemy-orb',impactAssetKey:'MARK',impactProfile:'void-seal',emphasis:1.08},
    HUNTER:{duration:735,contactMs:529,particleProfile:'hunter-shot',impactAssetKey:'IMPACT',impactProfile:'pierce-shards',emphasis:.94},
    VANGUARD:{duration:920,contactMs:662,particleProfile:'siege-shot',impactAssetKey:'SHOCK',impactProfile:'siege-shock',emphasis:1.16}
  });
  ...
  function card(record){
    ...
    if(RANGED_CARDS.has(key)){
      ...
      return{...event,...profile,category:'PROJECTILE',asset:BASE.ASSETS.PROJECTILE,impactAsset:authored(profile.impactAssetKey,BASE.ASSETS.SHOCK),pipeline:PIPELINES.PROJECTILE,priority:'P0'};
    }
  ...
  function enemy(record){
    ...
    if(PROJECTILE_ARCHETYPES.has(archetype)||event.asset?.motion==='TRAVEL'){
      ...
      return{...event,...profile,category:asset===BASE.ASSETS.ELITE_VANGUARD?'ELITE_VANGUARD':'PROJECTILE',asset,impactAsset:authored(profile.impactAssetKey,BASE.ASSETS.SHOCK),pipeline:PIPELINES.PROJECTILE,duration,contactMs:Math.round(profile.contactMs*eliteMultiplier),priority:'P0'};
    }
  ...
  const API=Object.freeze({...BASE,VERSION,PIPELINES,RANGED_CARDS,HEAVY_CARDS,PROJECTILE_ARCHETYPES,CARD_PROJECTILE_PROFILES,ENEMY_PROJECTILE_PROFILES,CARD_HEAVY_PROFILES,ENEMY_HEAVY_PROFILES,SIGNATURE_IMPACT_PROFILES,BOSS_IMPACT_PROFILES,card,enemy});
